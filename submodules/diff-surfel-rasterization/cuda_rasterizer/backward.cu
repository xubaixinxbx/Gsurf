...
renderCUDA(
	...
	const float3* __restrict__ sdf_grad_select,
	...
	float* __restrict__ dL_dnormal_sdf,
	...
	float* __restrict__ dL_dcolors)
{
	...
	__shared__ float3 collected_sdf_grad[BLOCK_SIZE];
	...
	if (inside) {
		...
		for (int i = 0; i < 3; i++) 
			dL_dnormal_sdf_2D[i] = dL_depths[(SDF_GRAD_OFFSET + i) * H * W + pix_id];
	...
	float last_normal_sdf[3] = { 0 };
	float accum_normal_sdf_rec[3] = {0};
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			collected_sdf_grad[block.thread_rank()] = sdf_grad_select[coll_id];
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			float3 sdf_grad = collected_sdf_grad[j];
			float normal_sdf[3] = {sdf_grad.x, sdf_grad.y, sdf_grad.z};
			...
			// Propagate sdf-gradients to per-Gaussian normals
			for (int ch = 0; ch < 3; ch++) {
				accum_normal_sdf_rec[ch] = last_alpha * last_normal_sdf[ch] + (1.f - last_alpha) * accum_normal_sdf_rec[ch];
				last_normal_sdf[ch] = normal_sdf[ch];
				// dL_dalpha += (normal_sdf[ch] - accum_normal_sdf_rec[ch]) * dL_dnormal_sdf_2D[ch]; // we remove this without affecting the alpha gradient
				atomicAdd((&dL_dnormal_sdf[global_id * 3 + ch]), alpha * T * dL_dnormal_sdf_2D[ch]);
			}
#endif
