	...
	bool prefiltered)
{
	...
	if (sdf_grad != nullptr) {
		float3* sdf_grad_temp = (float3*)sdf_grad;
		sdf_grad_select[idx] = {sdf_grad_temp[idx].x, sdf_grad_temp[idx].y, sdf_grad_temp[idx].z};
	} else {
		sdf_grad_select[idx] = {0.0f, 0.0f, 0.0f};
	}
	...
	float* __restrict__ out_others)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// Render sdf_normal map
			for (int ch=0; ch<3; ch++) N_SDF[ch] += normal_sdf[ch] * w;
#endif
	...
	if (inside)
	{
		...
		for (int ch=0; ch<3; ch++) out_others[pix_id + (SDF_GRAD_OFFSET+ch) * H * W] = N_SDF[ch];
#endif
	}
}
